...
export const createUser = catchAsync(async (body, profileImage) => {
  ...
  const { name, username, email, password, passwordConfirmation } = body;
  let { companyName, address, phone, role } = body;
  ...
  // Now you can retrieve the document with virtuals using lean
  const userWithVirtuals = await User.findById(user._id).lean({
    virtuals: true
  });
